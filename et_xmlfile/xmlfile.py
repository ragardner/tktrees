...
class _FakeIncrementalFileWriter(object):
    ...
    def element(self, tag, attrib=None, nsmap=None, **_extra):
        ...
        # __exit__ part
        ee = self._element_stack.pop()
        if self._element_stack:
            (parent := self._element_stack[-1]).append(self._top_element)
            self._top_element = parent
        else:
            self._file.write(tostring(ee))
            self._top_element = None
    ...
    def write(self, arg):
        ...
        elif self._top_element is not None:
            self._top_element.append(arg)
        elif not self._have_root:
            self._file.write(tostring(arg))
        else:
            raise LxmlSyntaxError()
